# Metaprogramming Ruby Lab

Практические эксперименты по книге *Metaprogramming Ruby (Paolo Perrotta)*.

Цель:
- пройти книгу до конца;
- запускать все примеры руками;
- исследовать интересные механизмы Ruby глубже;
- сохранять собственные эксперименты.

## Progress

### Part I — Metaprogramming Ruby
- [x] Chapter 1 — The M Word
- [x] Chapter 2 — Monday: The Object Model
- [ ] Chapter 3 — Tuesday: Methods
- [ ] Chapter 4 — Wednesday: Blocks
- [ ] Chapter 5 — Thursday: Class Definitions
- [ ] Chapter 6 — Friday: Code That Writes Code
- [ ] Chapter 7 — Epilogue

### Part II — Metaprogramming in Rails
- [ ] Chapter 8 — Preparing for a Rails Tour
- [ ] Chapter 9 — The Design of Active Record
- [ ] Chapter 10 — Active Support's Concern Module
- [ ] Chapter 11 — The Rise and Fall of alias_method_chain
- [ ] Chapter 12 — The Evolution of Attribute Methods
- [ ] Chapter 13 — One Final Lesson

### Part III — Appendixes
- [ ] Appendix A1 — Common Idioms
- [ ] Appendix A2 — Domain-Specific Languages
- [ ] Appendix A3 — Spell Book
